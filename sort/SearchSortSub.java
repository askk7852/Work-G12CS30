...
public class SearchSort {
	...
	public static void lineSearch(int[] number, int fInt) {
		...
		for(int x = 0; x < number.length; x++) {
			if(number[x] == fInt) {
				counter++;
				System.out.println("The number " + fInt + " is located at index " + x + ".");
			}	
		}
		if(counter == 0)
			System.out.println("The number " + fInt + " in not in the array.");
	}
	public static void binSearch(int[] number, int fInt) {
	    int low = 0;
	    int high = number.length - 1;
	    while (low <= high) {
	        int mid = (low + high) / 2;
	        if (number[mid] == fInt) {
	            System.out.println("The number " + fInt + " is located at index" + mid + ".");;
	        } else if (number[mid] < fInt) {
	            low = mid + 1;
	        } else {
	            high = mid - 1;
	        }
